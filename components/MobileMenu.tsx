      ...
      <div className={`fixed top-0 right-0 h-full w-64 bg-[var(--background)] z-50 transform transition-transform duration-300 ease-in-out ${isOpen ? "translate-x-0" : "translate-x-full"}`}>
        <div className="flex flex-col h-full pt-20 px-6 gap-2">
          {visibleLinks.map((link) => (
            <Link key={link.href} href={link.href} className={`text-2xl py-4 flex gap-1.5 items-center border-b border-[var(--foreground)] ${isActive(link.href) ? "text-[var(--color-accent)]" : "text-white"}`} onClick={() => setIsOpen(false)}>
              <NavigationIcon name={link.icon} className={isActive(link.href) ? "stroke-[var(--color-accent)]" : "stroke-white"} />
              {link.label}
            </Link>
          ))}
          <Link href="/apply" className="text-2xl flex gap-1.5 items-center py-4 text-[var(--color-accent)] border-b border-[var(--foreground)] hover:text-white transition-colors duration-300" onClick={() => setIsOpen(false)}>
